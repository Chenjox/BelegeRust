use std::{fs::File, error, path::Path, io::Write, ops::Mul};

use linfa_linalg::{cholesky::InverseC, qr::{QR, QRInto}};
...
impl GebrochenRational {
    ...
    fn get_linear_isomorphism(&self) -> (Array2<f64>,Array2<f64>) {
        ...
        loop {
            ...
            let s_0 = if j==n-2 { (r_2[n-j-2])/r_1[n-j-2] } else { (r_2[n-j-2]-s_1*r_1[n-j-3])/r_1[n-j-2] };
            // r_j
