...
pub const EULER_MASCHERONI: f64 = 0.577215664901532860606512090082402431042159335939923598805767234884867726777664670936947063291746749;
...
impl LogNormal {
  ...
  pub fn cdf(&self, x: f64) -> f64 {
    ...
    return self.normal.cdf(trans_x);
  }
  ...
  pub fn pdf(&self, x: f64) -> f64 {
    ...
    return self.normal.pdf(trans_x) * 1.0/(self.sigma_nn*(x - self.lower_value));
  }

  pub fn inverse_nataf_transformation(&self, y: f64) -> f64 {
    return ((y + self.mean_nn)*self.sigma_nn).exp() + self.lower_value;
  }
  pub fn inverse_nataf_transformation_dx(&self, y: f64) -> f64 {
    return ((y + self.mean_nn)*self.sigma_nn).exp() * self.sigma_nn;
  }
}
