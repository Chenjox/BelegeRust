fn rosenbrock(input_vec: &Vec<f64>) -> f64 {
    ...
    for i in 0..input_vec.len() - 1 {
        running_sum += 100.0 * (input_vec[i + 1] - input_vec[i].powi(2)).powi(2)
            + (1.0 - input_vec[i]).powi(2);
    }
    return running_sum;
}
...
fn gradient_sensitivity_sj(min: f64, max: f64, order: i32) -> f64 {
    ...
    let a2 = 1.0 / 5.0
        * (161_000.0 * diff_of_squares(min, max, 1)
            - 320_000.0 / 2.0 * diff_of_squares(min, max, 2))
