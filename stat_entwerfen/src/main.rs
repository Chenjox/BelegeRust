...
use bleche::{Schraubenverbindung, Stahlblech};
...
fn auflagerkraefte(einzellast: f64, pos_last: f64, laenge: f64) -> [f64; 2] {
  let left_druck =
    einzellast * ((laenge - pos_last) / laenge).powi(2) * (1.0 + (2.0 * pos_last) / laenge);
  let right_druck =
    einzellast * (pos_last / laenge).powi(2) * (1.0 + (2.0 * (laenge - pos_last)) / laenge);
...
fn LF_zwei_einzellasten_moment(
  x_wert: f64,
  laenge: f64,
  pos_last_1: f64,
  pos_last_2: f64,
  ...
  return moment_bei_einzellast(einzellast, pos_last_1, laenge, x_wert)
    + moment_bei_einzellast(einzellast, pos_last_2, laenge, x_wert);
}
fn LF_zwei_einzellasten_querkraft(
  x_wert: f64,
  laenge: f64,
  pos_last_1: f64,
  pos_last_2: f64,
  ...
  return querkraft_bei_einzellast(einzellast, pos_last_1, laenge, x_wert)
...
fn main() {
  ...
  let LF1_moment_stirn = LF_zwei_einzellasten_moment(
    drittelspunkt,
    laenge,
    pos_last,
    laenge - pos_last,
  ...
  let LF1_moment_mitte = LF_zwei_einzellasten_moment(
    laenge / 2.0,
    laenge,
    pos_last,
    laenge - pos_last,
  ...
  let LF1_querkraft_mitte = LF_zwei_einzellasten_querkraft(
    laenge / 2.0,
    laenge,
    pos_last,
    laenge - pos_last,
  ...
  let LF1_querkraft_stirn = LF_zwei_einzellasten_querkraft(
    drittelspunkt,
    laenge,
    pos_last,
    laenge - pos_last,
  ...
  println!(
    "{},{},{},{}",
    LF1_moment_mitte, LF1_querkraft_mitte, LF1_moment_stirn, LF1_querkraft_stirn
  );

  let welast = LF1_moment_mitte * 100.0 / festigkeit * 1.1;
  println!(
    "Benötigtes elastisches Widerstandmoment: \n W_el = {}",
    welast
  );
